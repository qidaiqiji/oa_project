...
export default {
  ...
  effects: {
    *getDetail({ payload }, { put, call, select }) {
      ...
      const { pcInfo, xcxInfo } = yield select(state => state.aTypeModel);
      ...
      function checkHasJsonInfo(data) {
        ...
        data.adviceGoods = data.adviceGoods
          ? data.adviceGoods
          : { goodsList: [], isShowTitle: 1, bgColor: '#dd3450' };
    ...
    *submitInfo(_, { put, call, select }) {
      const { xcxInfo, pcInfo, activeKey, pageId, edit, showPc, showXcx } = yield select(
        state => state.aTypeModel,
      );
      ...
      function checkIsSelected(data) {
        ...
        data.jsonInfo &&
          data.jsonInfo.map(item => {
            item.isChecked = item.isChecked ? 1 : 0;
          });
        data.jsonInfo =
          data.jsonInfo &&
          data.jsonInfo.filter(item => {
      ...
      function getInfoList(data) {
        ...
        data.adviceGoods.goodsList.map(item =>
          data.adviceGoods.goodsInfoList.push({
          ...
          data.jsonInfo.map(item => {
            if (
              ...
              item.goodsList.map(goods => {
                item.goodsInfoList.push({
                  goodsId: goods.goodsId,
                  sortOrder: goods.sortOrder ? goods.sortOrder : 0,
                });
              });
