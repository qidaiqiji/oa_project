...
class ATypeModel extends PureComponent {
  ...
  handleSubmit = () => {
    ...
    let extraLength = tempInfoType.rightNav.moduleList.some(
      item => item.newTitle && item.newTitle.length > 4,
    );
    if (extraLength) {
  ...
  handleUpload = (type, currentIndex, fileList) => {
    // console.log("uyy",fileList)
    ...
    //             info['bgImgUrl'] = fileList;
    //             break;
    //         case "banner":
    //             info['banner'] = fileList;
    //             break;
    //         case 'titleImg':
    //             if (currentIndex != null) {
    //                 // info.jsonInfo[currentIndex]['titleImg'] = response.data.imgs[0].imgId;
    //                 info.jsonInfo[currentIndex]['titleImgUrl'] = fileList;
    //                 break
    //             } else {
    //                 // info.allGoods['titleImg'] = response.data.imgs[0].imgId;
    //                 info.allGoods['titleImgUrl'] = fileList;
    //                 break
    //             }
    //         case 'adList':
    ...
    //             info.rightNav.ad.imgUrl = fileList;
    //             break;
    //     }
    // }
    // if (status === 'done') {
      ...
      tempAllValue.jsonInfo.map((item, index) => {
        if (item.type == 2) {
          item.adList.map((ad, adIndex) => {
            tempAllValue.jsonInfo[index]['adList'][adIndex] = Object.assign(
              tempinfo.jsonInfo[index]['adList'][adIndex],
              ad,
            );
          });
        }
      ...
      <PageHeaderWrapper title="A级活动页面">
        <Card loading={cardLoading}>
          <Tabs onChange={this.handleChangeTabs} defaultActiveKey={'onPC'} activeKey={activeKey}>
            ...
            {showXcx && (
              <Tabs.TabPane tab="小程序" key="onApplet">
                <CommonPart
                  data={xcxInfo}
                  type="xcx"
                  addItem={this.handleAdd}
