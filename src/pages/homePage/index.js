...
class HomePage extends React.Component {
  ...
  handleUpload = (currentIndex, fileList) => {
    const { dispatch, homePage } = this.props;
    const { infoDetail } = homePage;
    if (currentIndex != null) {
      infoDetail.diamondInfo[currentIndex].fileList = fileList;
    } else {
      infoDetail.baseInfo.logoUrl = fileList;
    }
    dispatch({
          ...
          <Card bordered loading={cardLoading}>
            <Form onSubmit={this.handleSubmit}>
              ...
              <Card bordered>
                      ...
                      {getFieldDecorator('baseInfo.color', {
                        ...
                        <AutoComplete
                          dataSource={colorMap.map(item => (
                            <Select.Option value={item.color} style={{ background: item.color }}>
                              {item.color}
                            </Select.Option>
                          ))}
                        >
                ...
                <Row>
                  <Col span={12}>
                    ...
                    {infoDetail.baseInfo.logo && (
                      <ImgBox
                        src={infoDetail.baseInfo.logoUrl}
                        onDeleteImg={this.handleDeleteImg.bind(this, 'logo')}
                      />
                    )}
                    {!infoDetail.baseInfo.logo && (
                      <UploadBox
              ...
              <Row className={styles.title} type="flex" justify="space-between">
                <Col span={{ xxl: 4, xl: 6 }}>PC左侧分类</Col>
                <Col span={{ xxl: 1, xl: 2 }}>
                  <Button onClick={this.handleAddPcLeft}>新增</Button>
                </Col>
              </Row>
              ...
              <Card>
                <PCandWXNum
                  data={infoDetail.goodsCollectInfo}
                  keyWords="goodsCollectInfo"
                  getFieldDecorator={getFieldDecorator}
                />
              </Card>
      ...
      <Col span={2}>
        {index == 0 ? (
          <Button
            ...
            shape="circle"
            icon="plus"
            onClick={onAddItem.bind(this, 'nav', null, resetFields)}
          ></Button>
        ) : (
          <Button
            style={{ marginLeft: 10 }}
            shape="circle"
            icon="minus"
            onClick={onReduceItem.bind(this, 'nav', index, null, resetFields)}
          ></Button>
        )}
      </Col>
    </Row>
  ...
  return (
    <div className={styles.box}>
      <Row style={{ borderBottom: '1px dashed #e8e8e8' }} align="middle" type="flex">
        <Col span={5}>
          <Form.Item label="标题" labelCol={{ span: 7 }} wrapperCol={{ span: 14 }}>
        ...
        <Col span={5}>
          <Form.Item label="绑定分类" {...pcLeftLayout}>
            {getFieldDecorator(`leftCategory[${index}].mainCategory.catId`, {
              ...
              rules: [{ required: true, message: '请选择绑定分类' }],
            })(
              <Select>
                {Object.keys(firstClassCatMap).map(item => (
                  <Select.Option value={item}>{firstClassCatMap[item]}</Select.Option>
                ))}
              </Select>,
            )}
          </Form.Item>
        ...
        {index != 0 && (
          <Col span={2} push={7}>
            <div onClick={deleteItem.bind(this, index)} style={{ color: 'red', cursor: 'pointer' }}>
              删除
            </div>
          </Col>
        )}
      ...
      <Col span={2}>
        {subIndex == 0 ? (
          <Button
            ...
            shape="circle"
            icon="plus"
            onClick={onAddItem.bind(this, 'leftNav', index, resetFields)}
          ></Button>
        ) : (
          <Button
            style={{ marginLeft: 10 }}
            shape="circle"
            icon="minus"
            onClick={onReduceItem.bind(this, 'leftNav', index, subIndex, resetFields)}
          ></Button>
        )}
      </Col>
    </Row>
  ...
  return (
    <div className={styles.cardItem}>
      <Row>
        ...
        {!data.imgUrl && (
          <UploadBox index={index} onUpload={onUpload} onResourceUpload={onResourceUpload} />
        )}
      </Row>
        ...
        <Col span={24}>
          <div className={styles.container}>
            <div className={styles.staticBox}>
              <div className={styles.firstStaticBox} onClick={triggerShowBtn.bind(this, index)}>
                {data.type == 1 ? '按默认规则' : '按模块排序'}
              </div>
              {data.isShowBtn && (
                <div className={styles.staticBtn}>
                  <Button
                    ...
                    onClick={selectType.bind(this, index, 1)}
                  >
                    按默认规则
                  </Button>
                  <Button type="primary" size="small" onClick={selectType.bind(this, index, 2)}>
                    按模块排序
                  </Button>
                </div>
              )}
    ...
    <div className={styles.goodsItem}>
      ...
      <Form.Item label="pc链接" {...goodsLayout}>
        {getFieldDecorator(`channelInfo.info[${index}].pcLink`, {
          ...
          rules: [{ required: true, message: '请输入pc链接' }],
        })(<Input />)}
      </Form.Item>
      <Form.Item label="小程序路由" {...goodsLayout}>
        {getFieldDecorator(`channelInfo.info[${index}].xcxLink`, {
          initialValue: data.xcxLink,
          rules: [{ required: true, message: '请输入小程序路由' }],
        })(<Input />)}
      </Form.Item>
      <Form.Item label="商品" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }}>
        {data.goodsList &&
          data.goodsList.map((item, subIndex) => {
            return (
              <ImgBox src={item.img} index={index} onDeleteImg={onDeleteImg} subIndex={subIndex} />
            );
          })}
